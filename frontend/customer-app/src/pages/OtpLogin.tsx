...
export const OtpLogin: React.FC = () => {
  ...
  const handleVerifyOTP = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      const response = await api.post('/auth/otp/verify', { phone, code: otpCode });
      if (response.data.status === 'success') {
    ...
    >
      ...
      >
        ...
        <p style={{ color: 'var(--text-muted)', fontSize: '0.95rem', marginBottom: '32px' }}>
          {!otpSent ? 'Enter your mobile number to sign in or register' : 'Enter the 6-digit code sent to your phone'}
        </p>

        {!otpSent ? (
          <form onSubmit={handleSendOTP} style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
            <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
              <label style={{ fontSize: '0.85rem', fontWeight: 600, color: 'var(--text-slate)' }}>Phone Number</label>
              <input
                type="text"
                value={phone}
          ...
          <form onSubmit={handleVerifyOTP} style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
            <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
              <label style={{ fontSize: '0.85rem', fontWeight: 600, color: 'var(--text-slate)' }}>OTP Code</label>
              <input
                type="text"
                maxLength={6}
                value={otpCode}
