...
export const Dashboard: React.FC = () => {
    ...
    const fetchDriverStats = async () => {
      try {
        ...
        if (ordersRes.data.status === 'success') {
          ...
          const active = allOrders.find((o: any) => o.status === 'out_for_delivery');
          if (active) {
            setActiveJob(active);
          }
        }
  ...
  const handleToggleOnline = async () => {
    ...
    try {
      const response = await api.put('/delivery/status', { is_online: nextOnlineState });
      if (response.data.status === 'success') {
        setIsOnline(nextOnlineState);
        toast.success(`You are now ${nextOnlineState ? 'ONLINE' : 'OFFLINE'} for deliveries.`);
      }
    } catch (err) {
  ...
  const handleAcceptJob = async (orderId: string) => {
    try {
      const response = await api.put(`/orders/${orderId}`, { status: 'out_for_delivery' });
      if (response.data.status === 'success') {
  ...
  const handleDeliverJob = async (orderId: string) => {
    try {
      const response = await api.put(`/orders/${orderId}`, { status: 'delivered' });
      if (response.data.status === 'success') {
        setActiveJob(null);
  ...
  return (
    <div style={{ padding: '40px', maxWidth: '1000px', margin: '0 auto' }}>
      ...
      >
        ...
        >
          <div>
            <h1 style={{ fontSize: '2.2rem', marginBottom: '8px' }}>Logistics Panel</h1>
      ...
      {activeJob && (
        ...
        >
          <h2 style={{ fontSize: '1.6rem', marginBottom: '16px', color: 'var(--accent-violet)' }}>
            Active Delivery Job
          </h2>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <div>
              <strong style={{ fontSize: '1.2rem' }}>Order #{activeJob.order_number}</strong>
      ...
      <h2 style={{ fontSize: '1.6rem', marginBottom: '24px' }}>Available Pickup Jobs</h2>
        ...
        {jobs.map((job) => (
          ...
          >
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px' }}>
              ...
              <span style={{ color: 'var(--accent-orange)', fontSize: '0.85rem', fontWeight: 700 }}>
                READY FOR PICKUP
              </span>
            </div>

            <div style={{ fontSize: '0.95rem', color: 'var(--text-slate)', marginBottom: '24px', flexGrow: 1 }}>
              <div>From: <strong>{job.restaurant_name}</strong></div>
              <div>To: <strong>{job.street_address}</strong></div>
            </div>
