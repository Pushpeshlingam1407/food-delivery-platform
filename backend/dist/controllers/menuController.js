...
async function verifyRestaurantOwner(userId, restaurantId) {
    const [rows] = await db_js_1.default.query("SELECT owner_id FROM restaurants WHERE id = ?", [restaurantId]);
...
async function createMenuCategory(req, res) {
    ...
    try {
        ...
        await db_js_1.default.query("INSERT INTO menu_categories (id, restaurant_id, name, sort_order, is_active) VALUES (?, ?, ?, ?, TRUE)", [id, restaurant_id, name, sort_order]);
...
async function getMenuCategories(req, res) {
    ...
    try {
        const [rows] = await db_js_1.default.query("SELECT * FROM menu_categories WHERE restaurant_id = ? AND deleted_at IS NULL ORDER BY sort_order ASC", [restaurantId]);
...
async function updateMenuCategory(req, res) {
    ...
    try {
        const [rows] = await db_js_1.default.query("SELECT restaurant_id FROM menu_categories WHERE id = ?", [id]);
        ...
        const isOwner = await verifyRestaurantOwner(req.user.userId, categories[0].restaurant_id);
        if (!isOwner && req.user.role !== "admin") {
            return res
                .status(403)
                .json({ status: "error", message: "Forbidden: Access denied" });
        ...
        params.push(id);
        await db_js_1.default.query(`UPDATE menu_categories SET ${updates.join(", ")} WHERE id = ?`, params);
        return res
            .status(200)
            .json({
            status: "success",
...
async function deleteMenuCategory(req, res) {
    ...
    try {
        ...
        await db_js_1.default.query("UPDATE menu_categories SET deleted_at = NOW() WHERE id = ?", [id]);
        return res
            .status(200)
            .json({
            status: "success",
...
async function createMenuItem(req, res) {
    ...
    const { restaurant_id, category_id, name, description, price, is_veg = true, preparation_time = 15, available_quantity = 0, unlimited = true, image_url, } = req.body;
    ...
    try {
        ...
        const menuId = crypto_1.default.randomUUID();
        const inventoryId = crypto_1.default.randomUUID();
        const connection = await db_js_1.default.getConnection();
        try {
            await connection.beginTransaction();
            // 1. Insert Menu Item
            await connection.query(`INSERT INTO menus (id, restaurant_id, category_id, name, description, price, is_veg, is_available, preparation_time) 
         VALUES (?, ?, ?, ?, ?, ?, ?, TRUE, ?)`, [
                menuId,
                restaurant_id,
                category_id,
            ...
            // 2. Insert Inventory
            await connection.query("INSERT INTO inventory (id, menu_id, available_quantity, unlimited) VALUES (?, ?, ?, ?)", [inventoryId, menuId, available_quantity, unlimited]);
            // 3. Insert Image if provided
            if (image_url) {
                const imageId = crypto_1.default.randomUUID();
                await connection.query("INSERT INTO menu_images (id, menu_id, image_url, is_primary) VALUES (?, ?, ?, TRUE)", [imageId, menuId, image_url]);
            }
...
async function updateMenuItem(req, res) {
    ...
    const { id } = req.params;
    const { name, description, price, is_veg, is_available, preparation_time } = req.body;
    try {
        const [rows] = await db_js_1.default.query("SELECT restaurant_id FROM menus WHERE id = ?", [id]);
        const menus = rows;
        if (menus.length === 0) {
            return res
                .status(404)
        ...
        const isOwner = await verifyRestaurantOwner(req.user.userId, menus[0].restaurant_id);
        if (!isOwner && req.user.role !== "admin") {
            return res
                .status(403)
                .json({ status: "error", message: "Forbidden: Access denied" });
        ...
        params.push(id);
        await db_js_1.default.query(`UPDATE menus SET ${updates.join(", ")} WHERE id = ?`, params);
        return res
            .status(200)
...
async function deleteMenuItem(req, res) {
    ...
    try {
        ...
        await db_js_1.default.query("UPDATE menus SET deleted_at = NOW() WHERE id = ?", [id]);
        return res
            .status(200)
            .json({
            status: "success",
