...
export async function createAddress(req, res) {
  ...
  const { address_type, street_address, landmark, city, state, postal_code, latitude, longitude, is_default } = req.body;

  if (!street_address || !city || !state || !postal_code || latitude === undefined || longitude === undefined) {
  ...
  try {
    ...
    try {
      ...
      if (defaultFlag === 1) {
        await connection.query('UPDATE addresses SET is_default = FALSE WHERE user_id = ?', [req.user.userId]);
      }
      ...
      await connection.query(
        `INSERT INTO addresses (id, user_id, address_type, street_address, landmark, city, state, postal_code, latitude, longitude, is_default) 
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
        [addressId, req.user.userId, address_type || 'home', street_address, landmark || null, city, state, postal_code, latitude, longitude, defaultFlag]
      );
    ...
    return res.status(201).json({ status: 'success', message: 'Address created successfully', data: { id: addressId } });
  } catch (error) {
...
export async function updateAddress(req, res) {
  ...
  try {
    const [rows] = await pool.query('SELECT user_id FROM addresses WHERE id = ? AND deleted_at IS NULL', [id]);
    if (rows.length === 0) {
    ...
    if (address_type) { updates.push('address_type = ?'); params.push(address_type); }
    if (street_address) { updates.push('street_address = ?'); params.push(street_address); }
    ...
    if (postal_code) { updates.push('postal_code = ?'); params.push(postal_code); }
    ...
    try {
      ...
      if (updates.length > 0) {
        params.push(id);
        await connection.query(`UPDATE addresses SET ${updates.join(', ')} WHERE id = ?`, params);
      }
...
export async function deleteAddress(req, res) {
  ...
  try {
    ...
    await pool.query('UPDATE addresses SET deleted_at = NOW(), is_default = FALSE WHERE id = ?', [id]);
