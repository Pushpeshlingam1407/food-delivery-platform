...
export async function createCmsPage(req, res) {
  ...
  if (!slug || !title || !content) {
    return res.status(400).json({ status: 'error', message: 'Missing required CMS fields' });
  }
  ...
  try {
    ...
    await pool.query(
      `INSERT INTO cms_pages (id, slug, title, content, meta_title, meta_description, is_published) 
       VALUES (?, ?, ?, ?, ?, ?, ?)`,
      [id, slug, title, content, meta_title || null, meta_description || null, is_published ? 1 : 0]
    );

    return res.status(201).json({ status: 'success', message: 'CMS page created successfully', data: { id, slug } });
  } catch (error) {
...
export async function updateCmsPage(req, res) {
  ...
  try {
    ...
    params.push(id);
    await pool.query(`UPDATE cms_pages SET ${updates.join(', ')} WHERE id = ?`, params);

    return res.status(200).json({ status: 'success', message: 'CMS page updated successfully' });
  } catch (error) {
...
export async function deleteCmsPage(req, res) {
  ...
  try {
    ...
    return res.status(200).json({ status: 'success', message: 'CMS page deleted successfully' });
  } catch (error) {
