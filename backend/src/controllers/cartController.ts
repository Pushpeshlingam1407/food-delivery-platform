...
export async function getCart(req: Request, res: Response) {
  ...
  try {
    ...
    const cartMeta = (cartRows as any[])[0];
    ...
    // Load checkout configurations from system settings
    const [settingsRows] = await pool.query(
      "SELECT key_name, value FROM system_settings",
    );
    const settingsMap: { [key: string]: number } = {};
    (settingsRows as any[]).forEach((setting) => {
    ...
    // Validate coupon code if requested
...
export async function addToCart(req: Request, res: Response) {
  ...
  try {
    ...
    // 2. Fetch active cart restaurant metadata
    const [cartRows] = await pool.query(
    ...
    try {
      ...
      // Update cart restaurant reference
...
export async function updateCartItem(req: Request, res: Response) {
  ...
  try {
    ...
    // If cart is now empty, clean up restaurant_id reference
...
export async function removeFromCart(req: Request, res: Response) {
  ...
  try {
    ...
    // Clean up restaurant reference if empty
    const [countRows] = await pool.query(
      "SELECT COUNT(*) as count FROM cart_items WHERE cart_id = ?",
      [cartId],
    );
