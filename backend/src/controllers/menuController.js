...
export async function createMenuCategory(req, res) {
  ...
  if (!restaurant_id || !name) {
    return res.status(400).json({ status: 'error', message: 'Restaurant ID and category name are required' });
  }
...
export async function updateMenuCategory(req, res) {
  ...
  try {
    const [rows] = await pool.query('SELECT restaurant_id FROM menu_categories WHERE id = ?', [id]);
    ...
    return res.status(200).json({ status: 'success', message: 'Menu category updated successfully' });
  } catch (error) {
...
export async function deleteMenuCategory(req, res) {
  ...
  try {
    ...
    await pool.query('UPDATE menu_categories SET deleted_at = NOW() WHERE id = ?', [id]);
    return res.status(200).json({ status: 'success', message: 'Menu category soft-deleted successfully' });
  } catch (error) {
...
export async function createMenuItem(req, res) {
  ...
  if (!restaurant_id || !category_id || !name || price === undefined) {
    return res.status(400).json({ status: 'error', message: 'Missing required menu fields' });
  }
  ...
  try {
    ...
    try {
      ...
      await connection.query(
        `INSERT INTO menus (id, restaurant_id, category_id, name, description, price, is_veg, is_available, preparation_time) 
         VALUES (?, ?, ?, ?, ?, ?, ?, TRUE, ?)`,
        [menuId, restaurant_id, category_id, name, description, price, is_veg, preparation_time]
      );
    ...
    return res.status(201).json({ status: 'success', data: { menuId, name, price } });
  } catch (error) {
...
export async function updateMenuItem(req, res) {
  ...
  try {
    const [rows] = await pool.query('SELECT restaurant_id FROM menus WHERE id = ?', [id]);
    const menus = rows;
    if (menus.length === 0) {
    ...
    if (is_veg !== undefined) { updates.push('is_veg = ?'); params.push(is_veg); }
    ...
    params.push(id);
    await pool.query(`UPDATE menus SET ${updates.join(', ')} WHERE id = ?`, params);
...
export async function deleteMenuItem(req, res) {
  ...
  try {
    ...
    return res.status(200).json({ status: 'success', message: 'Menu item soft-deleted successfully' });
  } catch (error) {
